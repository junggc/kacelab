...
<script type="text/javascript">
    ...
    function send_() {
       ...
       // memberRow의 개수만큼 반복문을 돌리면서 enName값을 콤마로 구분하여 하나의 문자열로 만든다.
        ...
        jQuery("#sortTableForm").attr("action", "./project_proc.php");
...
<input type="hidden" id="mode" name="mode" value="sort"/>
    ...
    <ul class="line_t">
		<?
		$sql = "select * from TB_project where 1=1   order by sort_num asc,idx desc ";
		$result= mysqli_query($dbconn,$sql);
		while($row=mysqli_fetch_array($result)){
		?>
      <li class="memberRow" enName="<?=$row["idx"]?>"><?=$row["name_ko"]?></li>
	  <?}?>
